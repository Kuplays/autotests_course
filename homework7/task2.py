...
class PersonInfo:
    ...
    def __init__(self, person_name, person_age, *person_departments):
        """
        :param person_name: Имя
        :param person_age: Возвраст
        :param person_departments: департаменты от головного до частного
        """

        self.person_name = person_name
        self.person_age = person_age
        self.person_departments = list(person_departments)

    def short_name(self):
        """
        Возвращает сокращенное имя фамилию
        :return: строка Фамилия И.
        """

        return f'{self.person_name.split()[1]} {self.person_name[0]}.'

    def path_deps(self):
        """
        Возвращает сокращенный путь от головного до конкретного департамента
        :return: строка
        """

        return ' --> '.join(self.person_departments)
    ...
    def new_salary(self):
        ...
        1337*Возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
        (регистр имеет значение "А" и "а" - разные буквы)
        :return: число, новая зп после индексации (вот бы в жизни так)
        """

        count_dict = {}
        max_chars_sum = 0
        for char in ''.join(self.person_departments):
            if char in count_dict:
                count_dict[char] += 1
            else:
                count_dict[char] = 1
        for i in range(3):
            max_chars_sum += count_dict.pop(max(count_dict, key=count_dict.get))
        return PersonInfo.INDEXATION_CONST * self.person_age * max_chars_sum
