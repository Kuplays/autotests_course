...
def everything_for_your_cat(cats_data):
    organized_data = {}
    our_str = ''
    for single_tuple in cats_data:
        owner_key = f'{single_tuple[2]} {single_tuple[3]}'
        cat_record = (single_tuple[0], single_tuple[1])
        if owner_key not in organized_data:
            organized_data[owner_key] = []
        organized_data[owner_key].append(cat_record)
    for owner_key in organized_data:
        our_str += f'{owner_key}: '
        for cat_record in organized_data[owner_key]:
            our_str += f'{cat_record[0]}, {cat_record[1]}; '
        our_str = our_str[0:len(our_str) - 2] + '\n'
    return our_str
